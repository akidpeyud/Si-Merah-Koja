...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        /* --- ALERT --- */
        #globalSuccessAlert { position: fixed; top: 30px; left: 50%; transform: translateX(-50%); background-color: #10b981; color: white; padding: 16px 24px; border-radius: 8px; box-shadow: 0 10px 25px -5px rgba(16, 185, 129, 0.4); z-index: 99999; display: flex; align-items: center; gap: 12px; font-weight: 600; font-size: 14px; animation: slideDownCenter 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
        ...
        .btn-action { display: inline-flex; align-items: center; justify-content: center; width: 100%; padding: 7px 10px; border-radius: 6px; font-weight: 700; font-size: 11px; border: none; text-decoration: none; transition: all 0.2s; cursor: pointer; color: white; }
            ...
            @if(Auth::user()->role === 'user' || Auth::user()->role === 'super_user')
                ...
                <div class="collapse show" id="collapsePencegahan" data-bs-parent="#sidebarAccordion">
                    <div class="sidebar-submenu">
                        ...
                        <!-- MENU EDUKASI DI SINI -->
                        <a href="/internal/pencegahan/kelola-edukasi" class="sidebar-item active"><i class="fas fa-bullhorn"></i> Kelola Edukasi & Sosialisasi</a>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="fw-bold text-dark">{{ $p->institusi }}</div>
                                    <div class="text-muted" style="font-size: 11px;">Kec. {{ $p->kecamatan }}</div>
                                </td>
                                <td>
                                    <div class="fw-bold text-primary">{{ $p->nama_pemohon }}</div>
                                    <a href="https://wa.me/{{ preg_replace('/^0/', '62', $p->no_kontak) }}" target="_blank" class="text-success text-decoration-none" style="font-size: 11px; font-weight:600;"><i class="fab fa-whatsapp"></i> {{ $p->no_kontak }}</a>
                                </td>
                                <td>
                                    <div class="fw-bold" style="color: #ec4899;">{{ \Carbon\Carbon::parse($p->tgl_kegiatan)->format('d F Y') }}</div>
                                ...
                                <td>
                                    ...
                                    @elseif($p->status_permohonan == 'Disetujui') <span class="badge-status status-disetujui">Disetujui</span>
                                    @else <span class="badge-status status-ditolak">Ditolak</span>
                                    @endif
                                </td>
                                <td class="no-print-col text-center">
                                    @if($p->surat_permohonan) <a href="{{ asset('storage/' . $p->surat_permohonan) }}" target="_blank" class="badge bg-danger text-decoration-none mb-1"><i class="fas fa-file-pdf"></i> Surat</a> @endif
                                    @if($p->syarat_lainnya) <br><a href="{{ asset('storage/' . $p->syarat_lainnya) }}" target="_blank" class="badge bg-secondary text-decoration-none"><i class="fas fa-paperclip"></i> Syarat</a> @endif
                                </td>
                                <td class="no-print-col">
                                    <div class="btn-action-group">
                                        <a href="/internal/pencegahan/kelola-edukasi/{{ $p->id }}" class="btn-action bg-detail"><i class="fas fa-search me-1"></i> Detail</a>
                                        <button type="button" class="btn-action bg-status" data-bs-toggle="modal" data-bs-target="#modalStatus{{ $p->id }}"><i class="fas fa-edit me-1"></i> Status</button>
                                        <a href="/internal/pencegahan/kelola-edukasi/{{ $p->id }}?auto_print=true" target="_blank" class="btn-action bg-cetak"><i class="fas fa-print me-1"></i> Cetak</a>
                                    </div>
                                </td>
                            </tr>
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title fw-bold" style="font-size: 16px;">Update Status Edukasi</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form action="/internal/pencegahan/kelola-edukasi/update-status/{{ $p->id }}" method="POST">
                                            @csrf
                                            <div class="modal-body text-start">
                                                <p class="mb-3 text-muted" style="font-size: 13px;">Ubah status pengajuan jadwal dari <strong>{{ $p->institusi }}</strong>.</p>
                                                <div class="mb-3">
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted"><i class="fas fa-folder-open mb-2" style="font-size: 30px; color:#cbd5e1;"></i><br>Belum ada pengajuan Edukasi & Sosialisasi.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
