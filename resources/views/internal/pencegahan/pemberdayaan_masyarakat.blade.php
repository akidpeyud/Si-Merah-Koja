...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* TABEL SOSIALISASI (HEADER TINGKAT) */
        ...
        .table-detailed th { font-size: 11px; font-weight: 700; padding: 16px 15px; white-space: nowrap; text-transform: uppercase; border-right: 1px solid var(--ink-3); border-bottom: 1px solid var(--ink-3); vertical-align: middle; }
        .table-detailed .th-group { border-bottom: 1px solid var(--ink-3) !important; text-align: center; }
        ...
        /* STAT CARDS (UNTUK TAB SEMUA DATA) */
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; }
        .stat-card { display: flex; flex-direction: column; align-items: flex-start; padding: 24px; background: #fff; border-radius: var(--r-md); border: 1px solid var(--line); box-shadow: var(--shadow-xs); transition: box-shadow .2s ease, transform .2s ease; text-decoration: none !important; }
        .stat-card:hover { box-shadow: var(--shadow-sm); transform: translateY(-2px); border-color: #cbd5e1; }
        .stat-ico { width: 44px; height: 44px; border-radius: var(--r-sm); display: grid; place-items: center; font-size: 1rem; margin-bottom: 16px; transition: transform .2s ease; background: var(--navy-tint); color: var(--navy); }
        .stat-card:hover .stat-ico { transform: scale(1.05); }
        .stat-title { font-size: .75rem; font-weight: 700; letter-spacing: .04em; text-transform: uppercase; color: var(--steel); margin-bottom: 6px; }
        .stat-value { font-family: var(--font-display); font-weight: 700; font-size: 2.2rem; line-height: 1; color: var(--ink); }
    </style>
</head>
<body>
    ...
    <main class="content">
        <div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <a href="/internal/pencegahan/pemberdayaan-masyarakat/create" class="btn text-white fw-bold d-flex align-items-center gap-2" style="background-color: var(--navy); padding: 9px 16px; border-radius: 8px;">
        ...
        <!-- ================= JIKA DI TAB "SOSIALISASI" (ATAU DEFAULT LAINNYA) ================= -->
        @else
            <div class="section-heading">
                <span class="section-heading-ico"><i class="fas fa-users"></i></span>
                <h3>Data Sosialisasi & Edukasi</h3><span class="line"></span>
            </div>
                ...
                <table class="table-detailed table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="fw-bold text-dark">{{ $item->kelurahan }}</td>
                            <td>{{ $item->kecamatan }}</td>
                            <td class="text-center fw-bold">{{ $item->peserta_perempuan }}</td>
                            <td class="text-center fw-bold">{{ $item->peserta_laki_laki }}</td>
                            <td class="text-center">
                                @if($item->foto_video)
                                    <a href="/uploads/pemberdayaan/{{ $item->foto_video }}" target="_blank" style="color: var(--info); text-decoration: none; font-weight: 600;">
