...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --green: #16a34a; --blue: #2563eb; --blue-d: #1d4fd6; --steel: #5b6c7f; --line: #dbe2ea;
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag { display: inline-flex; align-items: center; gap: 6px; padding: 5px 10px; border-radius: 8px; font-size: .74rem; font-weight: 700; background: var(--paper); border: 1px solid var(--line); color: var(--steel); white-space: nowrap; }
        .tag i { font-size: .78rem; color: var(--blue); }
        .tag .val { color: var(--ink); }

        .img-wrap { display: inline-block; padding: 4px; border: 1px solid var(--line); border-radius: var(--r-sm); background: var(--paper); }
        .img-prasarana { width: 150px; height: 100px; object-fit: cover; border-radius: 7px; transition: transform .2s; }
        .img-wrap:hover .img-prasarana { transform: scale(1.03); }
...
<div class="shell">
        ...
        @if(in_array(Auth::user()->role, ['sapra', 'user', 'super_user']))
            ...
            <details class="side-group" {{ Request::is('sapra*') ? 'open' : '' }}>
                ...
                <div class="side-sub">
                    ...
                    <!-- ACTIVE KARENA HALAMAN INI PRASARANA MAKO -->
    ...
    <main class="content">
        ...
        <div class="page-toolbar">
            <div>
                <h1>Data Prasarana Pemadam Kebakaran</h1>
                <p>Manajemen dokumentasi visual prasarana Pemadam kebakaran.</p>
            </div>
            <div class="toolbar-actions">
                <label class="search">
                    <span class="sr-only" style="position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);">Cari prasarana</span>
                    <i class="fas fa-search"></i>
                    <input type="text" id="searchInput" placeholder="Cari prasarana..." autocomplete="off">
                </label>
                <button type="button" class="btn btn-primary" data-open="dlgTambah"><i class="fas fa-plus"></i> Tambah data</button>
                <a href="/sapra/prasarana-mako/cetak-pdf" class="btn btn-outline red"><i class="fas fa-file-pdf"></i> PDF</a>
            </div>
        </div>
        ...
        @foreach($posPemadam as $pos)
            ...
            <div class="panel" id="panel-{{ $pos->id_pos }}" role="tabpanel" @if(!$isActive) hidden @endif @if($isActive) data-print-active @endif>
                ...
                <!-- Kartu info pos -->
                <div class="info-card" style="display: flex; flex-wrap: wrap; align-items: center; gap: 10px 26px; background: #fff; border: 1px solid var(--line); border-left: 4px solid var(--blue); border-radius: var(--r-md); padding: 18px 22px; margin-bottom: 18px; box-shadow: var(--shadow-xs);">
                    <h2 style="font-family: var(--font-display); font-weight: 800; font-stretch: 90%; font-size: 1.05rem; letter-spacing: -0.01em; text-transform: uppercase; flex-basis: 100%; color: var(--ink);">{{ $pos->nama_pos }}</h2>
                    <span style="display: flex; align-items: center; gap: 9px; font-size: .87rem; color: var(--steel); font-weight: 600;"><i class="fas fa-map-marker-alt" style="color: var(--signal); width: 16px; text-align: center;"></i> {{ $pos->alamat ?? 'Alamat belum diatur' }}</span>
                    <span style="display: flex; align-items: center; gap: 9px; font-size: .87rem; color: var(--steel); font-weight: 600;">
                        <i class="fas fa-map" style="color: var(--signal); width: 16px; text-align: center;"></i> Kode map:
                        @if($pos->kode_map)
                        ...
                        <table class="data-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:45%">Jenis prasarana</th>
                            ...
                            <tbody>
                                ...
                                <tr class="data-row">
                                    ...
                                    <td class="data-name">
                                        <div class="cell-name">{{ $item->jenis_prasarana }}</div>
                                        @if($item->luas_bangunan)
                                            <div class="tag-row">
                                                <span class="tag"><i class="fas fa-expand-arrows-alt"></i> Luas: <span class="val">{{ $item->luas_bangunan }}</span></span>
                                            </div>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td class="c">
                                        <div class="row-actions">
                                            <button type="button" class="icon-btn edit" data-open="dlgEdit{{ $item->id_prasarana }}" aria-label="Edit"><i class="fas fa-pen"></i></button>
                                            
                                            <form action="/sapra/prasarana-mako/delete/{{ $item->id_prasarana }}" method="POST" style="margin:0;" onsubmit="return confirm('Yakin ingin menghapus prasarana ini? Gambar akan ikut terhapus permanen.');">
                                                @csrf @method('DELETE')
                                ...
                                <!-- Dialog Edit -->
                                <dialog class="sheet" id="dlgEdit{{ $item->id_prasarana }}">
                                    <div class="sheet-head">
                                        <h2>Edit Data Prasarana</h2>
                                    ...
                                    <form action="/sapra/prasarana-mako/update/{{ $item->id_prasarana }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="sheet-body">
                                            <div>
                                                <label class="f-label" for="id_pos{{ $item->id_prasarana }}">Pilih lokasi / pos</label>
                                                <select class="f-input" id="id_pos{{ $item->id_prasarana }}" name="id_pos" required>
                                            ...
                                            <div>
                                                <label class="f-label" for="jenis{{ $item->id_prasarana }}">Jenis prasarana</label>
                                                <input class="f-input" type="text" id="jenis{{ $item->id_prasarana }}" name="jenis_prasarana" value="{{ $item->jenis_prasarana }}" required>
                                            </div>
                                            <div>
                                                <label class="f-label" for="luas{{ $item->id_prasarana }}">Luas tanah / bangunan <span class="f-optional">(opsional)</span></label>
                                                <input class="f-input" type="text" id="luas{{ $item->id_prasarana }}" name="luas_bangunan" value="{{ $item->luas_bangunan }}" placeholder="Cth: 150 M² atau 2 Hektar">
                                            </div>
                                            <div>
                                                <label class="f-label" for="gambar{{ $item->id_prasarana }}">Ganti gambar <span class="f-optional">(opsional)</span></label>
                                                <input class="f-input" type="file" id="gambar{{ $item->id_prasarana }}" name="gambar" accept="image/*">
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="cell-empty">Belum ada data prasarana untuk {{ $pos->nama_pos }}.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
...
<dialog class="sheet" id="dlgTambah">
    ...
    <form action="/sapra/prasarana-mako/store" method="POST" enctype="multipart/form-data">
        ...
        <div class="sheet-body">
            ...
            <div>
                <label class="f-label" for="tambahJenis">Jenis prasarana</label>
                <input class="f-input" type="text" id="tambahJenis" name="jenis_prasarana" placeholder="Contoh: Gedung Kantor Utama" required>
            </div>
            <div>
                <label class="f-label" for="tambahLuas">Luas tanah / bangunan <span class="f-optional">(opsional)</span></label>
                <input class="f-input" type="text" id="tambahLuas" name="luas_bangunan" placeholder="Cth: 150 M² atau 2 Hektar">
            </div>
            <div>
